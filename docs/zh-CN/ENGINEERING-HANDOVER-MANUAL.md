...
- **聊天**：请求鉴权 → 创建/认领持久回合 → 规划有预算的上下文 → RAG/Agent →
  模型网关 → 流式返回 → 用户/助手消息一起持久化。失败或取消不会留下孤立正式消息。
...
1. 阅读根目录 `AGENTS.md`、[架构总纲](root/ARCHITECTURE.md)和相关设计文档。
...
- 生成参考：运行 `python -m scripts.generate_docs`，不要手改 `docs/generated/`。
...
3. 使用 AI 问答创建会话；查看流式回答和知识来源；在历史页搜索、重命名或归档。
4. 在模拟面试选择主题、难度和题数；逐题回答、查看评分/参考答案；中途离开后可恢复。
5. 在能力画像查看分数、趋势和薄弱项；在学习计划生成、完成和复习任务。
6. 在设置中管理头像、主题、提醒、密码和恢复码。
...
管理员 Token 只能从管理员登录接口取得。`APP_API_KEY` 是部署层附加保护，不能替代
用户身份和所有权校验。
