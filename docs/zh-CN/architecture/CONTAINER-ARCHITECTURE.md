...
app: FastAPI + built Vue
  ...
  |-- user-files: avatars, resumes, temporary review audio
...
worker
  ...
  |-- user-files + model provider: resume/review analysis
  `-- transcription provider: consent-gated audio transcription
...
- `worker`：执行可恢复的知识导入、简历分析、音频转写和面试复盘任务；
- `postgres`：生产权威业务数据库；
- `redis`：跨实例协调和非权威缓存；
- `qdrant`：版本化私人知识索引；
...
持久卷分别承载数据库、Redis、Qdrant、Grafana、Prometheus、应用数据、知识文件和
用户文件。启用简历或复盘处理时，API与Worker必须挂载同一用户文件卷。容器更新不能
隐式删除卷。
